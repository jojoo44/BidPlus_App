...
const String _openAiApiKey = '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';
...
Future<Map<String, dynamic>> _computeAiScoreWithDetails({
  ...
  try {
    ...
    for (final entry in weights.entries) {
      ...
      total += score * weight;
      debugPrint('=== $criterionName: $score × $weight = ${score * weight} ===');
    }
...
class _ContractorRFPDetailsScreenState
  ...
  Future<void> _loadRFP() async {
    try {
      final data = await supabase.from('RFP').select().eq('rfpID', widget.rfpId).single();
      if (mounted) {
  ...
  Future<void> _checkIfSubmitted() async {
    try {
      ...
      final data = await supabase.from('proposals').select('*, RFP(*)')
  ...
  Future<void> _notifyManager(String contractorName) async {
    try {
      ...
      final managerData = await supabase.from('User')
          .select('notificationsEnabled').eq('id', managerId).single();
          ...
          Future<void> pickFileForCriterion(String criterionName) async {
            ...
            try {
              ...
              for (final picked in result.files) {
                ...
                await supabase.storage.from('proposal_attachments').uploadBinary(path, fileBytes);
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (criteria.isNotEmpty) ...[
                    ...
                    const Text('Upload one or more PDFs for each criterion', style: TextStyle(color: Colors.grey, fontSize: 12)),
                              ...
                              child: Container(
                                ...
                                child: isLoadingThis
                                    ...
                                    : Row(
                                        ...
                                        children: [
                                          Icon(files.isEmpty ? Icons.upload_file_outlined : Icons.add,
                                              color: files.isEmpty ? Colors.grey : primaryBlue, size: 16),
                                          const SizedBox(width: 6),
                                          Text(files.isEmpty ? 'Upload PDF for $criterionName' : '+ Add another file',
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: Text('$totalUploaded file(s) uploaded  •  AI will read & score each',
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: isSubmitting ? null : () async {
                        ...
                        try {
                          ...
                          debugPrint('=== COMMENTS: $criteriaResponse ===');
                          debugPrint('=== FINAL SCORE: $finalScore ===');
                          ...
                          final userData = await supabase.from('User').select('username').eq('id', userId).single();
                        ...
                        Container(
                          ...
                          child: Wrap(
                            ...
                            children: _criteriaNames.map((name) => Container(
                              ...
                              decoration: BoxDecoration(
                                color: primaryBlue.withOpacity(0.15), borderRadius: BorderRadius.circular(20),
