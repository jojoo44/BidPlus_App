...
class _ProposalsListScreenState extends State<ProposalsListScreen> {
  ...
  Future<void> _loadProposals() async {
    try {
      ...
      // 2. جيب الـ usernames بشكل منفصل
      final userIds = proposals
          .map((p) => p['submitterUserId'])
          .where((id) => id != null)
          .toSet()
          .toList();
      ...
      for (final uid in userIds) {
        try {
          final u = await supabase
              ...
              .eq('id', uid)
              .single();
          usernameMap[uid.toString()] = u['username'] ?? 'Unknown';
        } catch (_) {}
      }

      // 3. أضف الـ username لكل proposal
      final enriched = proposals.map((p) {
        final uid = p['submitterUserId']?.toString() ?? '';
        return <String, dynamic>{
          ...p,
          'contractorName': usernameMap[uid] ?? 'Unknown',
        };
      }).toList();
  ...
  void _onSearch() {
    ...
    setState(() {
      _filtered = _proposals.where((p) {
        final name = (p['contractorName'] ?? '').toLowerCase();
