...
class _ContractorDashboardScreenState
  ...
  // ── Current Projects من جدول Contract
  Future<void> _loadCurrentProjects() async {
    ...
    try {
      ...
      // جيب الـ proposals المقبولة للكونتراكتر
      final proposals = await supabase
          .from('proposals')
          .select('RFP, proposedPrice')
      ...
      // جيب الـ Contracts المرتبطة
      final contracts = await supabase
          .from('Contract')
          .select()
          .inFilter('paymentID', rfpIds)
          .eq('status', 'Active');

      // أضف عنوان الـ RFP لكل contract
      final enriched = <Map<String, dynamic>>[];
      for (final contract in contracts) {
        final rfpId = contract['paymentID'];
        try {
  ...
  void _subscribeRealtime() {
    _realtimeChannel = supabase
        ...
        .onPostgresChanges(
          ...
          schema: 'public',
          table: 'Contract',
          callback: (payload) => _loadCurrentProjects(),
        )
        .subscribe();
  }
  ...
  Widget _homeBody() {
    return SafeArea(
      child: RefreshIndicator(
        onRefresh: () async {
          await _loadCurrentProjects();
          await _loadPublishedRFPs();
        },
        color: Colors.blue,
                  ...
                  ? Container(
                      ...
                      child: const Column(
                        children: [
                          ...
                          SizedBox(height: 4),
                          Text(
                            'Projects appear here after signing a contract',
                            style: TextStyle(
